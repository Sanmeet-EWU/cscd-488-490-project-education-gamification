export class DialogueManager {
    ...
    createDialogueBox() {
      ...
      this.dialogueContainer = this.scene.add
        .container(this.boxX + this.boxWidth / 2, containerY, [
    ...
    typeText(text) {
      ...
      if (this.textTimer) this.textTimer.remove(false);
    ...
    showResponses(responses) {
      ...
      responses.forEach((resp, index) => {
        ...
        let border = this.scene.add
          .rectangle(0, 0, option.width + 20, option.height + 10)
    ...
    adjustResponsePositions() {
      if (!this.dialogueContainer) return;
      const startY = this.dialogueText.y + this.dialogueText.height + 20;
      this.responseTexts.forEach((option, i) => {
        option.setPosition(this.dialogueText.x, startY + i * 40);
      });
    }
