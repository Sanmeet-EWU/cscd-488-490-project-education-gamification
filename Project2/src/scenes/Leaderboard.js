...
export class Leaderboard extends BaseScene {
    ...
    async create() {
        ...
        this.audioController.startMainMenuMusic(this);
    }
        ...
        setTimeout(() => {
            ...
            if (this.entries?.length) {
                this.entries.forEach((entry, index) => {
                    if (entry?.active) entry.setPosition(width / 2, height * (0.3 + index * 0.1));
                });
            }
        }, 50);
    }
...
const leaderboardEntry = (rank, username, score, scene, x, y) => {
    const panelWidth = 500;
    const panelHeight = 50;
    let backgroundColor = rank === 1 ? 0xC9B037 : rank === 2 ? 0xB4B4B4 : rank === 3 ? 0xA0522D : 0x4e342e;
    const background = scene.rexUI.add.roundRectangle(0, 0, panelWidth, panelHeight, 20, backgroundColor);
    const rankText = scene.add.text(0, 0, `${rank}.`, { fontFamily: 'Inknut Antiqua', fontSize: '24px', color: '#ffffff', stroke: '#000000', strokeThickness: 2 });
    const usernameText = scene.add.text(0, 0, username, { fontFamily: 'Inknut Antiqua', fontSize: '24px', color: '#ffffff', stroke: '#000000', strokeThickness: 2 });
    const scoreText = scene.add.text(0, 0, score, { fontFamily: 'Inknut Antiqua', fontSize: '24px', color: '#ffffff', stroke: '#000000', strokeThickness: 2 });
