...
export class Act5Scene7 extends BaseGameScene {
    constructor() {
        super('Act5Scene7');
    ...
    preload() {
        if (!this.textures.exists('background_act5scene7')) {
            this.load.svg('background_act5scene7', 'assets/act5/scene7.svg', { width: 2560, height: 1440 });
        }

        if (!this.cache.json.exists('Act5Scene7Data')) {
            this.load.json('Act5Scene7Data', 'SceneDialogue/Act5Scene7.json');
        }
        ...
        if (!this.cache.audio.exists('act5scene7music')) {
            this.load.audio('act5scene7music', 'assets/audio/act5scene7.mp3');
        }
    ...
    create(data) {
        ...
        this.nextSceneKey = 'Act5Scene8Part1';
        this.dialogueStarted = false;
        this.dialogueFullyComplete = false;
        this.transitionActive = false;
        this.soliloquyStarted = false;

        const requiredAssets = ['background_act5scene7'];
        ...
        if (this.textures.exists('background_act5scene7')) {
            this.background = this.add.image(0, 0, 'background_act5scene7')
                .setOrigin(0, 0)
        ...
        if (this.audioController && this.cache.audio.exists('act5scene7music')) {
            this.audioController.playMusic('act5scene7music', this, { volume: 1, loop: true });
        }
        ...
        if (this.isCutscene && this.dialogueManager) {
            this.dialogueManager.startDialogue("Act5Scene7", () => {
                this.switchScene('Act5Scene8Part1');
            });
        }
    ...
    setupSceneDialogue() {
        if (!this.cache.json.exists('Act5Scene7Data')) {
            console.error("Act5Scene7Data JSON not found");
            return;
        }

        try {
            const dialogueData = this.cache.json.get('Act5Scene7Data');
            const portraitMap = {
                "Macbeth": "Macbeth",
    ...
    update(time, delta) {
        ...
        if (this.player) {
            ...
            if (this.npcs["YoungSiward"] && !this.dialogueStarted) {
                const distToYoungSiward = Phaser.Math.Distance.Between(
                    this.player.x, this.player.y,
                    this.npcs["YoungSiward"].x, this.npcs["YoungSiward"].y
                );
                if (distToYoungSiward < 120) {
                    this.dialogueStarted = true;
                    this.startDialogue("YoungSiward");
                }
            }
    ...
    onResize(gameSize) {
        if (!this.scene.isActive('Act5Scene7')) return;
