...
export class PauseMenu extends BaseScene {
    ...
    init(data) {
      ...
      // Important: Tell AudioController we're pausing a game scene
      this.audioController = this.sys.game.globals.audioController;
      if (this.audioController && this.gameScene) {
          this.audioController.setGameScenePaused(this.gameScene.scene.key);
      }
  }
    ...
    create() {
        ...
        // REMOVED: Do not start main menu music in pause menu
        // this.audioController = this.sys.game.globals.audioController;
    ...
    repositionUI({ width, height }) {
        if (this.container?.active) this.container.setPosition(width / 2, height / 2);
    ...
    updateMusicVolume(value) {
        ...
        this.audioController.updateBackgroundVolume();
    }
    ...
    updateMusic(scene, musicButton) {
        if (!scene.audioController.musicOn) {
            ...
            scene.audioController.pauseMainMenuMusic();
        } else {
            musicButton.setTexture('checkedBox');
            scene.audioController.startMainMenuMusic(scene);
        }
    }

    updateSound(scene, soundButton) {
        soundButton.setTexture(scene.audioController.soundOn ? 'checkedBox' : 'uncheckedBox');
    }
}
